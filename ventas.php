...
if (isset($_POST['registrar_venta'])) {
    ...
    if ($stmt->execute()) {
        ...
        $ticket_data = urlencode(json_encode([
            'venta_id' => $venta_id,
            'folio' => $folio,
        ...
        echo "<script>window.location.href='ticket.php?data=$ticket_data';</script>";
        exit();
    }
}
...
<body>
            ...
            <form method="POST" onsubmit="return finalizarVenta()">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-top: 20px;">
                    <div class="form-group">
                        ...
                        <select name="metodo_pago" id="metodo_pago_select" required onchange="togglePagoEfectivo()">
                    ...
                    <div id="efectivo_fields" style="grid-column: 1 / -1; display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                        <div class="form-group" style="margin: 0;">
                            <label>Paga con: *</label>
                            <input type="number" step="0.01" name="pago_con" id="pago_con_input" oninput="calcularCambio()">
                        </div>
                        <div class="form-group" style="margin: 0;">
                            <label>Cambio:</label>
                            <input type="number" step="0.01" name="cambio" id="cambio_input" readonly style="background: #f8f9fa; font-weight: 700; color: #2ecc71;">
                        </div>
                    </div>
                </div>
    ...
    function finalizarVenta() {
        ...
        if (metodo === 'efectivo') {
            const pagoCon = parseFloat(document.getElementById('pago_con_input').value) || 0;
            if (pagoCon < total) {
    ...
    function calcularCambio() {
        ...
        document.getElementById('cambio_input').value = cambio >= 0 ? cambio.toFixed(2) : '0.00';
    }
    ...
    function togglePagoEfectivo() {
        ...
        if (metodo === 'efectivo') {
            efectivoFields.style.display = 'grid';
            document.getElementById('pago_con_input').required = true;
        } else {
            efectivoFields.style.display = 'none';
            document.getElementById('pago_con_input').required = false;
            document.getElementById('pago_con_input').value = '';
            document.getElementById('cambio_input').value = '';
        }
    }
